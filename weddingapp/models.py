...
class Guests(db.Model): 
    ...
    guest_addr= db.Column(db.String(255), nullable=True)
...
class Comments(db.Model):
    ...
    comment_date= db.Column(db.DateTime(), nullable=False, default=func.now())

    guest= db.relationship('Guests', backref='comments')
...
class Lga(db.Model):
    ...
    lga_state= db.relationship('State', backref="lgas")
...
class Orders(db.Model):
    ...
    order_totalamt= db.Column(db.Float())

    guest_info= db.relationship('Guests', backref='guest_orders')
...
class Order_details(db.Model):
    ...
    order_info= db.relationship('Orders', backref='order_item')
    item_info= db.relationship('Uniform', backref='uniform_orders')
...
class Payment(db.Model):
    ...
    pay_date=db.Column(db.DateTime(), default=func.now())
    pay_status= db.Column(db.Enum(Payment_status))
    pay_mode= db.Column(db.String(255))
    pay_feedback= db.Column(db.String(255))

    order_info= db.relationship('Orders', backref='order_payment')
    guest_info= db.relationship('Guests', backref='guest_payments')
